...
uses
  ...
  StdCtrls, Gauges, ExtCtrls, Unit2;
...
begin
  ...
  begin
    ...
    Form1.Gauge1.Progress:=round(100*cn/max5);
    write(f,j);
    inc(cn);
  end;
  close(f);
...
begin
 ...
 begin
  ...
  begin
    s:=0;
    if j and 31=0 then Form1.Gauge2.Progress:=round(100*j/Max4);
    for k:=i to Max4 do
      if C4[i,k] then s:=s+NFH4[FFF4[FH3[D4[j,0],D4[k,0]],FH3[D4[j,1],D4[k,1]]]];
    write(f,s);
    ct:=ct+s;
    ...
    inc(j);
    inc(p);
    while (j<=Max4) and (not C4[i,j]) do inc(j);
  end;
 end;
end;
...
begin
 ...
 begin
  ...
  begin
    s:=0;
    if i<>Max4 then Form1.Gauge2.Progress:=round(100*(j-i)/(Max4-i));
    for k:=0 to j do
      if C4[k,j] then s:=s+NFL4[FFF4[FL3[D4[i,0],D4[k,0]],FL3[D4[i,1],D4[k,1]]]];
    write(f,s);
    ct:=ct+s;
    cn:=cn+1;
    Form1.Label1.Caption:=FloatToStr(ct);
    Form1.Gauge1.Progress:=round(100*cn/7581);
    inc(j);
    inc(p);
    while (j<=Max4) and (not C4[i,j]) do inc(j);
  end;
...
begin
  ...
  if ct>1 then Time2:=(Date+Time-StartingTime)/ct*7828354
  else Time2:=0;
